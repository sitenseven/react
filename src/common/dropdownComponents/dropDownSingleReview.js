...
export const DropDownSingleReview = ({ name, data, getValue, getListID, label }) => {
          ...
          <ScrollView nestedScrollEnabled style={{ width: '100%' }}>
            ...
            <View
              ...
              }}>
              {
                list.map((item, index) => {
                  return (
                    <TouchableOpacity onPress={() => { getListID(item.id); selectValue(item.name) }} key={index} style={{ width: '100%', height: 35, justifyContent: 'center', alignItems: 'center' }} >
