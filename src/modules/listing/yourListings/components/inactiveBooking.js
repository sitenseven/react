...
const inactiveBooking = ({ img, data, navigation }) => {
  ...
  return (
    <View style={styles.container} >
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', padding: 16, paddingBottom: 6, marginTop: Platform.OS == 'android' ? 2 : 3 }} >
        <Image source={img} style={{ width: 70, height: 70, borderRadius: 4, }} />
        <View>
          <Text style={styles.titleStyle} >{title}</Text>
          <Text style={styles.labelStyle}>{listingType}</Text>
        </View>
      </View>
      <View style={{ width: '97%', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', marginBottom: 10 }}>
