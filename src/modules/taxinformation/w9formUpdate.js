...
const w9formUpdate = ({ navigation, route }) => {
  ...
  const [identificationNo, setIdentificationNo] = useState(detail.taxIdentificationNumber);
  ...
  const getSelectedProviderList = () => {
    const tempProvider = taxClassification.filter((item) => detail.taxClassification.includes(item.title))
  ...
  const saveTax = () => {
    if (bussinessName == '') {
      alert("Name of bussiness field should not be blank");
    } else if (ssn == '') {
      alert("SSN field should not be blank");
    } else if (address == '') {
        ...
        <ScrollView style={{ width: '100%' }}>
          <View style={{ width: '100%', alignItems: 'center' }}>
            ...
            <TouchableOpacity onPress={saveTax} style={{ width: '80%', height: 46, backgroundColor: '#2C99C6', borderRadius: 4, justifyContent: 'center', alignItems: 'center' }} >
              <Text style={{ color: '#FFFFFF', fontSize: widthPercentageToDP(3.5), fontFamily: FONTS.SFMedium }} >Update</Text>
            </TouchableOpacity>
